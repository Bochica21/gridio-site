...
This is the marketing website for **gridio**, a Brazilian digital product agency. It is a single self-contained file: `files/index.html`. There is no build system, no package manager, no framework — everything (HTML structure, CSS, JavaScript) lives inline in that one file.

## Running the site

Open `files/index.html` directly in a browser. No server or build step is required. For a local dev server you can use any static file server, e.g.:

```
npx serve files
# or
python -m http.server --directory files
```

## Architecture

Everything is in `files/index.html`:
...
Images are referenced in the HTML as `./assets/cases/puma/puma-*.jpg` (relative to `files/`), so they should live at `files/assets/cases/puma/`. The loose `.jpg` files currently at the `files/` root need to be moved there for the showcase section to render.
